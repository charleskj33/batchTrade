...
@Slf4j
public class NcsGdsTradesReader implements ItemReader<TradeDto> {
    ...
    public NcsGdsTradesReader(
        ...
        Map<String, TradeDto> allRecordsMap = filterAndDeduplicate(tradeDtoList);
        List<TradeDto> gdsTrades = updateClientBatchIds(allRecordsMap.values(), principalRepo, clientRepo);
        fileMetadata.setAllRecords(gdsTrades.stream()
                .map((TradeDto::getTradeRef))
                .toList());
        this.iterator = new ArrayList<>(gdsTrades).iterator();
    }
    ...
    private List<TradeDto> updateClientBatchIds(Collection<TradeDto> tradeDtos, PrincipalRepo principalRepo, ClientRepo clientRepo) {
